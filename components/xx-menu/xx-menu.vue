<template>
	<view>
		...
		<view class="major-box" :class="{ show: show }" :style="{ top: top + 'px' }">
			<view class="click-btn" v-if="!show" @tap="show = !show" draggable="true" @touchstart="touchstart" @touchmove.stop.prevent="touchmove">
				<view><&nbsp;快捷导航</view>
			</view>
			<view class="nav-box" v-if="show">
				<view class="nav-btn" v-for="(btn, index) in btnList" :key="index" @tap="clickBtn(btn.type)">
					<view class="nav-icon"><image class="icon" :src="btn.icon"></image></view>
