				...
				<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChangeMul">
					<block>
						...
						<picker-view-column v-if="fields=='day'">
							...
							<!-- <view  class="picker-item" v-for="(item,index) in dayArrEnd" :key="index">{{item}}</view> -->
						</picker-view-column>
					</block>
				</picker-view>
			</view>
		...
		methods:{
			...
			init(){
				...
				if(this.value[0]){
					...
					if(this.fields=='day')this.dayArr=this.getMonthDay(start.slice(0,4),start.slice(5,7)), this.dayArrEnd=this.getMonthDay(end.slice(0,4),end.slice(5,7),end.slice(8,10));
				}
